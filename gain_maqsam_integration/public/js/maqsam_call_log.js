(() => {
	...
	function getReadablePayloadSummary(payload, doc, summary, outcomeMeta, directionMeta) {
		...
		// Distinguish between "Maqsam is still processing" and "Maqsam doesn't
		// generate AI summaries for this call type / plan", so the user knows
		// whether to wait or contact Maqsam.
		const transcripts = findTranscriptArray(payload);
		if (!transcripts.length) {
			return {
				en: __("AI Summary not available for this call. Maqsam did not return a summary or transcript — likely because AI features aren't enabled for outbound calls on this account."),
				ar: "",
				_unavailable: true,
			};
		}
		return {
			en: __("Summary is still being generated by Maqsam. Refresh in a few minutes."),
			ar: "",
			_pending: true,
		};
	}
	...
	function renderSummaryLines(summary) {
		...
		// When the summary helper marks this as unavailable / pending, render it
		// as an explanatory note rather than as a real AI summary so the user
		// doesn't mistake it for transcribed content.
		if (summary?._unavailable || summary?._pending) {
			return `<div class="localized-summary-empty" dir="auto">${escapeHtml(en || ar)}</div>`;
		}
